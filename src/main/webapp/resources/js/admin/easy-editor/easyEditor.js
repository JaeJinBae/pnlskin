...
var easyConfig = {
	...
	//----------------------------------------------------
	filepath : "/_admin/easy-editor",	
	imgpath : "/_admin/easy-editor/img/easy",  //버튼 이미지경로
	...
	BtnTemplate  : {
		...
		all : ["preview","all","undo","redo","bar",
				"font","size","bar","color","hilite","bar","bold","italic",
				"underline","strike","bar","sup","sub","bar","clean","del","bar","outdent","indent",
				"bar","ul1","ul2","bar","left","center","right","justify","bar","hr","link",
				"unlink","bar","table","upload_image", "bar","source"],
		simple : ["source","bar","bold","color","hilite"],
		custom : ["font","size","bar","color","hilite","bar","bold","italic",
				"underline","strike","bar","left","center",
				"right","justify","bar","link","unlink","bar","table",
				"upload_image", "bar","source"]
	},
	version : "Beta",
	name : "easyWebEditor"
}
...
function easyEditor(id)
{
	...
	this.cfg.Btn = easyConfig.BtnTemplate["custom"];
	this.mode = "wysiwyg";
	this.btn = "";
	this.sel	  = null;
	this.range	  = null;
...
{	
	...
	css		+= " body,table,td{font:"+this.cfg.font+";color:"+this.cfg.color+"; font-size:12px; font-family:GUlim; }";
	/*css		+= " p{line-height:50%;}"; **/
	css		+= " p{ line-height:15px; }";
	css     += " html, body, p{ margin:0; padding:0; line-height:14px; font-family:굴림; font-size:12px; } ";
	this._iframe.css = css;
	this._doc.open();
	this._doc.write('<html><head><style type="text/css">'+css+'</style></head><body style="padding-top:10px">'+this._textarea.value+'</body></html>');
	this._doc.close();
