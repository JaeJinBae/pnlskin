...
$.widget("ui.sortable", $.extend({}, $.ui.mouse, {
	...
	_clear: function(event, noPropagation) {
		...
		this._SetCookieDiv();		//div 상태 쿠키에 굽기
		return true;
	...
	//재배열할 레이어의 값을 쿠키에 굽기
	_SetCookieDiv : function(){
		var options = { path : '/' , expires : 365}; //쿠키의 파기시간 365일로 설정

		for(var i=1 ; i < 4; i++){
			var DivChild = $("#column"+i).children();
			var CookieName = "column" + i; //쿠키변수명
			var DivLayer = ""; //column에 속해 있는 ID명들
			var division = ""; //쿠키변수 구분자

			$(DivChild).each(function(){
				var DivID = $(this).attr("id");
				if(DivID){
					DivLayer += division + DivID;
					division = "|";
				}
			});
//			$.cookie(CookieName, DivLayer , options);
//			$.cookie("RePosition", "yes", options);
			$.cookie(CookieName, DivLayer);
			$.cookie("RePosition", "yes");
		}//end for
	}
